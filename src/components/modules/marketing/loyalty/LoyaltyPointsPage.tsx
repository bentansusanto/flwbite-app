...
export default function LoyaltyPointsPage() {
  ...
  const stats = [
    { label: "Total Points Issued", value: "1,250,400", icon: <Coins size={20} />, color: "text-brand-600", bg: "bg-brand-100", trend: "+8.2%" },
    { label: "Points Redeemed", value: "450,200", icon: <Gift size={20} />, color: "text-purple-600", bg: "bg-purple-100", trend: "+12.5%" },
    { label: "Circulating Points", value: "800,200", icon: <Wallet size={20} />, color: "text-amber-600", bg: "bg-amber-100", trend: "-2.4%" },
  ];
  ...
  return (
    <div className="space-y-4 sm:space-y-6 bg-transparent">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-800 dark:text-white/90">Loyalty Points</h3>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Configure point rewards and track customer loyalty engagement.</p>
        </div>
      ...
      <div className="rounded-3xl border border-gray-100 bg-white shadow-theme-xs dark:border-white/5 dark:bg-gray-900/40 dark:backdrop-blur-md overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-bold text-gray-800 dark:text-white/90">Recent Point Transactions</h4>
          </div>
          <div className="flex gap-2">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50/80 dark:bg-white/[0.03] border-b border-gray-100 dark:border-white/5 text-[10px] font-semibold uppercase tracking-widest text-gray-400">
                ...
                <th className="px-6 py-4 text-right">Points Change</th>
      ...
      <Modal isOpen={isConfigOpen} onClose={() => setIsConfigOpen(false)} className="max-w-md">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-base font-bold text-gray-800 dark:text-white/90">Loyalty Rules</h4>
          </div>
        </div>
        <form onSubmit={handleSaveConfig} className="p-5 space-y-5">
          <div className="space-y-3">
            <Label>Earning Rate (IDR per 1 point)</Label>
            <div className="relative">
            ...
            <p className="text-[11px] text-gray-400">Set how many Rupiah customers need to spend to get 1 loyalty point.</p>
          </div>
          <div className="space-y-3 pt-2">
            <Label>Redemption Value (IDR per 1 point)</Label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">Rp</span>
              <InputField className="pl-12" value={config.redemption_value} onChange={e => setConfig({...config, redemption_value: e.target.value})} />
            </div>
            <p className="text-[11px] text-gray-400">Set the discount value for each 1 point redeemed.</p>
          </div>
          <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
            <Button type="button" variant="outline" className="flex-1" onClick={() => setIsConfigOpen(false)}>Cancel</Button>
