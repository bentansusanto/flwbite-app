...
export default function StockTakesPage() {
  const {
    ...
    branches,
    isLoadingBranches,
    filteredStockTakes, paginatedStockTakes, currentPage, setCurrentPage, pageSize, setPageSize,
    isLoadingStockTakes,
    handleCreate,
    handleUpdate,
    isCreating,
    ...
    <div className="space-y-4 sm:space-y-6 bg-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 dark:bg-gray-900/40 dark:backdrop-blur-md dark:border-white/5">
          <div className="w-12 h-12 bg-brand-50 rounded-xl flex items-center justify-center text-brand-600 dark:bg-brand-500/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 dark:bg-gray-900/40 dark:backdrop-blur-md dark:border-white/5">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-2.5 text-brand-600 font-semibold hover:bg-brand-50 rounded-xl transition-colors dark:hover:bg-brand-500/10"
        >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-white/5">
              {isLoadingStockTakes ? (
                ...
                paginatedStockTakes.map((take: any) => (
                  <tr key={take.id} className="hover:bg-gray-50/50 transition-colors group dark:hover:bg-gray-800/50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm font-semibold text-gray-700 dark:text-gray-300">
                      <div className="flex items-center gap-1.5 text-brand-600 bg-brand-50/50 px-2 py-1 rounded-md w-fit dark:bg-brand-500/10">
                        <Building2 className="w-3.5 h-3.5" />
                        {take.branch_name}
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-center text-sm font-bold text-gray-900 dark:text-white">{take.items_count || 0}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-center">
                      {getStatusBadge(take.status)}
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-right flex items-center justify-end gap-1">
                      {take.status === 'DRAFT' && (
                        <button
      ...
      <div className="flex flex-col gap-3 border-t border-gray-100 px-5 py-3.5 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center justify-center px-2 text-xs font-medium text-gray-700 dark:text-gray-300">
            {currentPage} / {Math.ceil(filteredStockTakes.length / pageSize) || 1}
          </div>
          <button
            onClick={() => setCurrentPage(p => p + 1)}
            disabled={currentPage >= Math.ceil(filteredStockTakes.length / pageSize)}
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 dark:border-gray-700 dark:hover:bg-gray-800 transition-colors"
          >
            <ChevronDown size={13} className="-rotate-90" />
          </button>
          <button
            onClick={() => setCurrentPage(Math.ceil(filteredStockTakes.length / pageSize))}
            disabled={currentPage >= Math.ceil(filteredStockTakes.length / pageSize)}
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 dark:border-gray-700 dark:hover:bg-gray-800 transition-colors"
          >
            <ChevronDown size={13} className="-rotate-90" />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                ...
                <div className="max-h-[350px] overflow-auto pr-2 -mr-2 space-y-3 no-scrollbar">
                             ...
                             {form.items.map((item, idx) => {
                               ...
                               return (
                                <tr key={idx} className="text-xs">
                                   <td className="whitespace-nowrap px-4 py-3 font-bold text-gray-700 dark:text-gray-300">{item.product_name}</td>
                                   <td className="whitespace-nowrap px-4 py-3 text-center">
                                      <span className={`font-bold ${diff !== 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                         {diff > 0 ? '+' : ''}{diff}
                                      </span>
                                   </td>
                                   <td className="whitespace-nowrap px-4 py-3 text-gray-500 italic font-medium dark:text-gray-400">{item.note || "-"}</td>
                                </tr>
                               );
                             })}
                          </tbody>
      ...
      <Modal isOpen={!!selectedTakeId} onClose={() => setSelectedTakeId(null)} className="max-w-2xl">
        <div className="p-4 sm:p-6 dark:bg-gray-900">
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100 dark:border-gray-800">
            <div className="w-12 h-12 bg-brand-50 rounded-2xl flex items-center justify-center dark:bg-brand-500/10">
                       ...
                       {selectedTake?.items?.length === 0 ? (
                         ...
                         selectedTake?.items?.map((item: any) => (
                            <tr key={item.id} className="dark:bg-gray-900/50">
                               ...
                               <td className="whitespace-nowrap px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-400">{item.expected_qty}</td>
                               <td className="whitespace-nowrap px-4 py-3 text-center font-bold text-gray-900 dark:text-white">{item.actual_qty}</td>
                               <td className="whitespace-nowrap px-4 py-3 text-center">
      ...
      <Modal
        ...
        className="max-w-md"
      >
        <div className="p-5 sm:p-8 text-center dark:bg-gray-900">
