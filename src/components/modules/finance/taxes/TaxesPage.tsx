  ...
  return (
    <div className="space-y-4 sm:space-y-6 bg-transparent">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-800 dark:text-white/90">Pajak</h3>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Atur dan kelola persentase pajak untuk bisnis Anda.</p>
        </div>
            ...
            <div key={tax.id} className="group relative rounded-2xl border border-gray-100 bg-white p-5 transition-all hover:border-brand-100 hover:shadow-theme-xl dark:border-white/5 dark:bg-gray-900/40 dark:backdrop-blur-md">
              ...
              <div className="mt-6 flex items-center justify-between border-t border-gray-50 pt-4 dark:border-gray-800">
                <div className="flex flex-col">
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Persentase</span>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} className="max-w-lg">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <h4 className="text-base font-bold text-gray-800 dark:text-white/90">{selectedTax ? "Edit Aturan Pajak" : "Buat Pajak Baru"}</h4>
          <p className="text-sm text-gray-400">Tentukan nama pajak dan persentase.</p>
        </div>
        <Formik
          initialValues={{
        ...
        >
          {({ values, errors, touched, handleChange, handleBlur, setFieldValue }) => (
            <Form className="p-5 space-y-4">
              ...
              <div>
                <Label required>Persentase (%)</Label>
                <InputField
                  type="number"
                  name="value"
                  placeholder="0"
              ...
              <div className="flex items-center gap-3 py-2">
                <Switch
                  label={values.is_active ? "Status: Aktif" : "Status: Tidak Aktif"}
                  checked={values.is_active}
                  onChange={(checked) => setFieldValue("is_active", checked)}
                />
              </div>
              <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                <Button type="button" variant="outline" className="flex-1" onClick={handleCloseModal} disabled={isLoading}>Batal</Button>
                <Button type="submit" className="flex-1" disabled={isLoading}>
                  {isLoading ? "Menyimpan..." : selectedTax ? "Perbarui" : "Buat"}
                </Button>
              </div>
            </Form>
          )}
      ...
      <AlertDialog
        ...
        description="Ini akan menghapus pajak dari semua transaksi di masa mendatang. Anda yakin?"
        confirmLabel="Hapus"
        cancelLabel="Batal"
        variant="danger"
